...
const els = {
  ...
  choiceShuffleBtn: document.querySelector("#choiceShuffleBtn"),
...
async function loadState() {
  ...
  state.favorites = await idbGet("favorites", {});
  await Promise.all([save(), saveTimer(), saveSyncQueue(), saveFavorites()]);
}
...
function categoryOf(question) {
  ...
  if (subject.id === "clinical" && source.includes("Ch.43-47(케이스)")) return "Ch.43-47(케이스)";
  if (subject.id === "clinical" && source.includes("Ch.44/45")) return "Ch.44/45";
...
async function saveFavorites() {
  await idbSet("favorites", state.favorites);
}
...
function isFavorite(question) {
  return Boolean(question && state.favorites[question.id]);
}
...
function filteredQuestions() {
  ...
  if (state.mode === "favorite") return questions.filter((q) => isFavorite(q));
...
function displayChoicesFor(question) {
  if (!question.choices?.length) return [];
  if (!state.choiceShuffle) {
    return question.choices.map((choice) => ({ ...choice, originalLabel: choice.label }));
  }

  const cached = state.choiceOrder[question.id];
  if (cached && cached.length === question.choices.length) return cached;

  const shuffled = shuffledCopy(question.choices).map((choice, index) => ({
    ...choice,
    originalLabel: choice.label,
    label: CHOICE_LABELS[index] || String(index + 1),
  }));
  state.choiceOrder[question.id] = shuffled;
  return shuffled;
}
...
function correctAnswer(question) {
  ...
  return correctLabelsFor(question).sort().join(",");
}

function correctLabelsFor(question) {
  if (question.trueFalse) return [question.trueFalse];
  if (!state.choiceShuffle) return [...question.correctLabels];
  return displayChoicesFor(question)
    .filter((choice) => question.correctLabels.includes(choice.originalLabel))
...
function paintChoiceResults(question) {
  ...
  document.querySelectorAll(".choice").forEach((button) => {
    ...
    button.classList.toggle("correct", correctLabels.includes(label) || question.trueFalse === label);
...
function selectChoice(label) {
  const question = currentQuestion();
  if (!question) return;
  if (correctLabelsFor(question).length > 1) {
...
function renderQuestion(question) {
  ...
  const displayChoices = displayChoicesFor(question);
  ...
  els.answerBox.textContent = `정답: ${correctAnswer(question)}`;
...
function renderStats() {
  ...
  const favorite = questions.filter((q) => isFavorite(q)).length;
  const categoryLabel = state.categoryId === "all" ? "전체" : state.categoryId;
  els.sourceMeta.textContent = `${subject.label} · ${categoryLabel} · ${questions.length}문항 · 즐겨찾기 ${favorite}`;
...
function renderList() {
  const titles = { due: "풀 문제", wrong: "틀린 문제", correct: "맞힌 문제", favorite: "즐겨찾기", all: "전체 문제" };
  ...
  els.questionList.classList.toggle("favorite-grid", state.mode === "favorite");
  pool.forEach((question) => {
    const button = document.createElement("button");
    const status = statusOf(question);
    button.type = "button";
    button.className = state.mode === "favorite" ? "favorite-card" : "list-item";
    button.innerHTML =
      state.mode === "favorite"
        ? favoriteCardHTML(question, status)
...
function favoriteCardHTML(question, status) {
  const choices = (question.choices || []).slice(0, 3).map((choice) => `<li>${escapeHtml(choice.label)} ${escapeHtml(choice.text)}</li>`).join("");
  return `
    <div class="favorite-card-head">
      <span class="badge ${status}">${statusLabel(status)}</span>
      <strong>★ ${escapeHtml(chapterOf(question))}</strong>
    </div>
...
function render() {
  ...
  els.choiceShuffleBtn.classList.toggle("active", state.choiceShuffle);
}

function renderFavoriteButton(question) {
  if (!question) {
    els.favoriteBtn.disabled = true;
    els.favoriteBtn.textContent = "☆ 즐겨찾기";
    els.favoriteBtn.classList.remove("active");
    return;
  }
  els.favoriteBtn.disabled = false;
  els.favoriteBtn.textContent = isFavorite(question) ? "★ 즐겨찾기" : "☆ 즐겨찾기";
  els.favoriteBtn.classList.toggle("active", isFavorite(question));
}
